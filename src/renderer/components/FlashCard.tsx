...
export default function FlashCard() {
  const { flashToday, flashMonth, recentCalls, monthStats, sessionStart } = useStore();

  const flashStream = useMemo(
    () => recentCalls.filter((c) => c.model === 'deepseek-chat' && c.source !== 'csv-import' && c.timestamp > sessionStart).slice(0, 5),
    [recentCalls, sessionStart]
  );

  const cacheRate = useMemo(() => {
    const totalPrompt = flashMonth.promptTokens;
    if (totalPrompt <= 0) return 0;
    return (flashMonth.cacheHitTokens / totalPrompt) * 100;
  }, [flashMonth]);

  const cacheMissToday = flashToday.promptTokens - flashToday.cacheHitTokens;
  const cacheMissMonth = flashMonth.promptTokens - flashMonth.cacheHitTokens;

  const efficiency = useMemo(() => {
    if (flashMonth.cost <= 0) return 0;
    return flashMonth.totalTokens / flashMonth.cost;
  }, [flashMonth]);

  const totalPercent = useMemo(() => {
    if (monthStats.cost <= 0) return 0;
    return (flashMonth.cost / monthStats.cost) * 100;
  }, [flashMonth.cost, monthStats.cost]);

  return (
    <div className="glass-card p-4 border-l-2 border-flash">
      <div className="flex items-center gap-2 mb-3">
        <Zap className="w-4 h-4 text-flash" />
        <h2 className="text-sm font-semibold">V4 Flash</h2>
        <span className="text-xs text-slate-500 ml-auto">deepseek-chat</span>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <span className="text-xs text-slate-500">今日 Tokens</span>
          <p className="text-lg font-bold text-white">{flashToday.totalTokens.toLocaleString()}</p>
          <span className="text-xs text-slate-600">{flashMonth.totalTokens.toLocaleString()} 本月</span>
        </div>
        <div>
          <span className="text-xs text-slate-500">今日费用</span>
          <p className="text-lg font-bold text-flash">¥{flashToday.cost.toFixed(6)}</p>
          <span className="text-xs text-slate-600">¥{flashMonth.cost.toFixed(4)} 本月</span>
        </div>
      </div>
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-white">命中 {flashToday.cacheHitTokens.toLocaleString()} / 未命中 {cacheMissToday.toLocaleString()}</span>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="text-flash">{totalPercent.toFixed(1)}%</span>
          </div>
          <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
            <div
              className="h-full bg-flash rounded-full transition-all duration-500"
